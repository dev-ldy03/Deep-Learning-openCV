...
int main() {
	...
	// 이미지 로드: 합성할 두 장의 원본 이미지를 읽어오는 로직
	// Read each image
	I1 = imread("Img01.jpg");
	I2 = imread("Img02.jpg");

	// 데이터 타입 변환 및 정규화: 블렌딩 연산 시 오버플로우 방지&정밀도를 위해 0~1 사이 float로 전환
	I1.convertTo(I1, CV_32FC3, 1.0 / 255);
	I2.convertTo(I2, CV_32FC3, 1.0 / 255);
	
	// corresponding points 설정: 두 이미지에서 같은 위치를 나타내는 특징점 좌표들
	...
	// Affine 행렬 계산: Least Squares Method 이용 -> 두 이미지 사이 기하학적 변환 행렬 계산
	...
	// 코너 워핑: I2의 네 모서리 좌표를 I1 좌표계로 투영하여 결과 영상의 크기를 가늠함
