...
int main() {
	...
	// NOTE: The following flags control the matching refinement strategy.
	// To reproduce Case 1/2/3 as described in the technical report,
	// these values were manually set per experiment prior to each build:
	bool crossCheck = true; // currently set for Case 3
	bool ratio_threshold = true; // currently set for Case 3
...
int nearestNeighbor(const Mat& vec, const vector<KeyPoint>& keypoints, const Mat& descriptors) {
	...
	for (int i = 0; i < descriptors.rows; i++) {
		...
		//	Fill the code
		//
		double dist = euclidDistance(vec, v);	// �Ÿ�
		if (dist < minDist) {	// find the nearest point
			minDist = dist;		// update
			neighbor = i;		// update
		}
	}
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (ratio_threshold) {
			// based on 1st closest neighbor~2nd closest neighbor ratio, find if it's a correct match

			double closest = euclidDistance(desc1, descriptors2.row(nn));
			double secondClosest = 1e6;
			for (int j = 0; j < descriptors2.rows; j++) {
				if (j == nn) {	// 1st closest neighbor is excluded from the calculation
					continue;
				}
				double temp = euclidDistance(desc1, descriptors2.row(j));	// finding the 2nd closest
				if (temp < secondClosest) secondClosest = temp;
			}
			if (closest >= RATIO_THR * secondClosest) {
				continue;	// skip if 1st and 2nd are not close enough
			}
		}
		...
		if (crossCheck) {
			...
			//	Fill the code
			//
			// Cross-checking ensures a match (i → nn) is only accepted if the reverse
			// match (nn → i) also holds, eliminating asymmetric/unreliable matches.
			Mat desc2 = descriptors2.row(nn); // descriptor of the matched keypoint in image2
			int rev = nearestNeighbor(desc2, keypoints1, descriptors1); // find the reverse match in image1
			if (rev != i) {
				continue;  // reject if reverse match does not point back to the original keypoint
		}
		}
