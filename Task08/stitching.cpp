...
void stitching(const Mat& img1, const Mat& img2, vector<Point2f>& srcPoints, vector<Point2f>& dstPoints, String estOpt, int k, int S);
...
void stitching(const Mat& img1, const Mat& img2, vector<Point2f>& srcPoints, vector<Point2f>& dstPoints, String estOpt, int k, int S) {	// srcPoints - dstPoints 대응점이므로 affine 행렬 M 계산
	// img2를 img1에 붙이는 코드
	...
	const double delta = 3;	// 임계값

	auto runStitchWithAffine = [&](const Mat& M12, const Mat& M21, const String& winSuffix) {
	...
	String leftWin = "Left Image " + winSuffix;
	String rightWin = "Right Image " + winSuffix;
	String resultWin = "result " + winSuffix;
	...
	if (estOpt == "case1") {	// 단순 Mx=b, outlier 포함한 벡터 전체로 계산하므로 성능 안 좋음
		...
		Mat M12 = cal_affine<float>(ptl_x, ptl_y, ptr_x, ptr_y, src_len);	// I1 -> I2 affine 행렬
		Mat M21 = cal_affine<float>(ptr_x, ptr_y, ptl_x, ptl_y, dst_len);	// I2 -> I1 affine 행렬
		runStitchWithAffine(M12, M21, "case1");
	}
	else {  // Mx=b + RANSAC
        for (int k_ransac : {3, 4}) {
            Mat M12 = cal_affine_RANSAC(dstPoints, srcPoints, k_ransac, S, delta);
            Mat M21 = cal_affine_RANSAC(srcPoints, dstPoints, k_ransac, S, delta);
            runStitchWithAffine(M12, M21, "case2 k=" + to_string(k_ransac));
        }
    }
}
...
Mat cal_affine_RANSAC(vector<Point2f>& srcPoints, vector<Point2f>& dstPoints, int k, int S, double delta) {
	...
	*/
	...
	// 4. bestInliers에 대해 Affine 행렬 M을 다시 계산
