...
#include <stdio.h>
#include <algorithm>	// for arrangement
#include <vector>
...
Mat Salt_pepper_noise_removal_Gray(const Mat input, int n, const char* opt) {
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			kernel = Mat::zeros(kernel_size * kernel_size, 1, input.type()); // initialize
			std::vector<G> kernel_vector = {};	// adjustkernel
			...
			if (!strcmp(opt, "adjustkernel")) {
				sort(kernel_vector.begin(), kernel_vector.end());	// sorting in ascending order
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			...
			if (!strcmp(opt, "adjustkernel")) {
				...
				median = floor(kernel_vector_r.size() / 2);	// since rgb kernel size is the same, only r
